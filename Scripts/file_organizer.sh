...
organize_files() {
        ...
        # Create folders for images, text, documents, and other files
        ...
        find $directory -type f ! \( -iname "*.jpg" -o -iname "*.png" -o -iname "*.txt" -o -iname "*.pdf" -o -iname "*.docx" -o -iname "*.xlsx" -o -iname "*.pptx" -o -iname "*.sh" \) -exec mv {} $directory/other/ \;
...
restore_files() {
        ...
        find "$directory/images" -type f \( -iname "*.jpg" -o -iname "*.png" \) -exec mv {} "$directory/" \;
        find "$directory/text" -type f -iname "*.txt" -exec mv {} "$directory/" \;
        find "$directory/documents" -type f \( -iname "*.pdf" -o -iname "*.docx" -o -iname "*.xlsx" -o -iname "*.pptx" \) -exec mv {} "$directory/" \;

        # Move files from the "other" folder back to their original locations
        find "$directory/other" -type f ! \( -iname "*.sh" \) -exec mv {} "$directory/" \;
